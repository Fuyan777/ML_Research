...
def showGragh(df_plot, cnt, face):
    ...
    plt.ylim([0, 30])
    # plt.show()
    plt.savefig("/Users/fuyan/Documents/siraisi_lab/B4/40_program/graph/%s/img%d.png" % (face, cnt))
    plt.close('all')
...
time_speak_path = "/Users/fuyan/Documents/siraisi_lab/B4/40_program/csv/speak-20201015/face_%s_speak_%s.csv" % (user, speak)
...
df_face = pd.DataFrame(face_feature, columns=[' timestamp', ' gaze_angle_x', ' gaze_angle_y', ' pose_Tx', ' pose_Ty', ' pose_Tz', ' y_62', ' y_66'])
cnt = 0

# 顔特徴のデータ格納
for speak_index in df_sp.index:
    timestamp_d = df_face[(df_face[' timestamp'] > start_time[speak_index]) & (df_face[' timestamp'] < end_time[speak_index])]
...
with open('/Users/fuyan/Documents/siraisi_lab/B4/40_program/csv/%s-output_feature_value_%s.csv' % (user, speak), 'w') as f:
    writer = csv.writer(f)
    writer.writerow(header)
    writer.writerows(feature_value_list)

# 特徴量csvの読み取り
feature_csv = pd.read_csv('/Users/fuyan/Documents/siraisi_lab/B4/40_program/csv/%s-output_feature_value_%s.csv' % (user, speak))
print(feature_csv)
