...
speak_type = [keep, change, non]

for speak in speak_type:
    ...
    for speak_index in df_sp.index:
        ...
        # 前のdfと結合
        # df_tmp = pd.concat([df_all_feature_sorted, df_tmp])
        ...
        # # 最後にheaderと結合
        # if speak_index == df_sp.index[-1]:
        #     tmp_speak_label = 0

        #     # ラベル付け
        #     if (speak == keep) or (speak == change):
        #         tmp_speak_label = 0
        #     else:
        #         tmp_speak_label = 1

        #     # df_add_label = df_union_data.insert(0, "speak", tmp_speak_label)

        #     df_header = pd.DataFrame(columns=common.feature_colums_reindex)
        #     df_feature = pd.concat([df_header, df_all_feature_sorted])
        #     df_feature.to_csv(
        #         file_path.feature_path
        #         + "/%s-output/feature_value_%s.csv" % (user, speak),
        #     )
